...
const createArticleQuerySpec = (where?: string): string => {
    return `SELECT * from c WHERE c.type = "articles" ${where ? `AND ${where} ` : ''}ORDER BY c.fetchedAt OFFSET 0 LIMIT 1`
}

export const getArticleDataForGivenDateTimeFromCosmosDB = async (container: Container, dateTime: Dayjs) => {
    const dateTimeIso = dateTime.toISOString();
    const dateTimePlusHourIso = dateTime.add(1, 'hour').toISOString();
    const querySpec: SqlQuerySpec = {
        query: createArticleQuerySpec(`c.fetchedAt >= "${dateTimeIso}" AND c.fetchedAt < "${dateTimePlusHourIso}"`),
    };
...
export const getOldestArticleDataFromCosmosDB = async (container: Container) => {
    const querySpec: SqlQuerySpec = {
...
export const addHistoricalDataToArticles = (latestDevToArticles, dayAgoData, weekAgoData, monthAgoData, oldestArticleData): IArticle[] => {
...
const getCombined = (key: 'comments' | 'reactions' | 'pageViews', articles: IArticle[]): IHistoricalData => {
...
export const getCombinedArticleStats = (articles: IArticle[]): { comments: IHistoricalData, reactions: IHistoricalData, pageViews: IHistoricalData } => ({
