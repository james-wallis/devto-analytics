...
const PageViewReaction = ({
    data: unfilteredData,
    reaction = false,
    daily = false,
}: IProps): JSX.Element => {
    const data = daily
        ? unfilteredData.filter(({ fetchedAt }) =>
    ...
    const graphData: IViewIncreaseGraph[] = data.slice(1).map(
        ({ fetchedAt, reactions, views }, i: number): IViewIncreaseGraph => {
    ...
    return (
        <ResponsiveContainer width="100%" height={300}>
            ...
            >
                ...
                <XAxis
                    dataKey="date"
                    tickFormatter={(date: string) => dayjs(date).format(daily ? 'H' : 'DD MMM')}
                />
                <YAxis />
